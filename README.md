...
### Web 界面（默认英文，多语言/中文适配后续迭代）

进入首页即**对话模式**（当前仅开放**Stealth（隐式）**模式），顶部居中显示 **Be water my friend.**，品牌 logo 为扁平化「?」标记，洞察使用**眼睛**意象图标（扁平线条风）。Web 需要**登录**：用户名（系统唯一、不区分大小写、5-64 位字母数字下划线）+ 密码 + 自动生成用户 id，昵称（显示名）独立设置；Google / 微信第三方登录为预留按钮（Coming soon）。数据按用户隔离（`<dataDir>/users/<userId>/`），首次注册自动迁移旧单用户数据。

登录后：左侧边栏含「New chat」「Settings」与**历史对话**（最新在上、更早在下；悬停可**重命名**或**归档**，归档仅从列表移除，数据仍保留用于分析）。首次使用引导设置**称呼**（Personal info）。输入框为可增高多行输入，底部工具栏右侧显示**当前具体模型名**（如 deepseek-v4-flash）、**上下文使用小圆环**（按模型实际最大输入上下文计算，悬浮显示进度比例与 tokens 总量）与**上箭头发送**按钮；键入 `/` 弹出工具模板，由 LLM 主持对应流程。输入框下方展示全局统计（Sessions / Insights / Persona，点击进入分析面板）；对话区右边框中部的**小凸起**可展开/收起右侧洞察栏（眼睛图标），并可展开到 Insights 完整页。**Settings** 为悬浮面板（点击外部或右上角 ✕ 返回），左侧分栏包含：**Personal info**（昵称、职业、年龄、性别、兴趣爱好）、**Model**（选择提供商后自动列出可选模型，含上下文窗口与成本；多个提供商的 API Key 会被记住，切换无需重新配置）、**Account**（当前账号 + 切换账号 / 登出）、**Share**（360° 反馈链接）、**System**（语言等，未实现置灰）。

**分享链接页（/f/<linkId>）**：独立公开页面，**无侧边栏、不展示原用户任何历史/档案数据**，仅包含反馈收集表单 + 显目引导按钮 **Start your own discovery journey →**（跳转登录）。反馈提交按 linkId 解析所属用户并写入其档案。
