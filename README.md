...
极简舒适风格。进入首页即**对话模式**（当前仅开放**隐式**模式），顶部居中显示 **Be water my friend.**，品牌 logo 为扁平化的「?」标记（自省/德尔斐之问，非镜子），设置与洞察均采用扁平线条风格图标。左侧边栏包含「新对话」「设置」与**历史对话**（最新/今天在上、更早的在下；悬停可**重命名**或**归档**，归档仅从列表移除，数据仍保留用于分析）。首次使用会引导设置**称呼**（个人基础信息，可在设置页修改）。输入框为**可增高**的多行输入，底部工具栏右侧显示**当前具体模型名**（如 deepseek-v4-flash）、**上下文使用小圆环**（按模型实际最大输入上下文计算，悬浮显示进度比例与 tokens 总量）与**上箭头发送**按钮；键入 `/` 弹出工具模板（V-T-D / SIGN / SWOT / 成就事件 / 兴趣矩阵 / 每日回馈 / 从业分析 / 人生设计），由 LLM 主持对应流程。输入框下方展示全局统计（会话数 / 洞察数 / 画像版本，点击进入分析面板）；对话区右边框中部的**小凸起**可展开/收起右侧洞察栏（最近洞察、画像叙事、转折点），并可展开到「洞察」主页面。**「⚙️ 设置」**页选择提供商后自动列出可选模型（下拉选择），多个提供商的 API Key 会被记住，切换提供商无需重新配置。
